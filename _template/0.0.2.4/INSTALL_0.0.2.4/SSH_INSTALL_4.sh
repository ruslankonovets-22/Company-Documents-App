...
#
# ⚠️  ИСПРАВЛЕНИЯ v0.0.2.7:
# - developer_mode включается ПЕРЕД созданием сайта
# - server_script_enabled включается ПЕРЕД созданием сайта
# - Исправлен порядок Folder Structure Template (nested set)
# - Упрощена логика ожидания создания сайта (без зависаний)
# - Полный вывод сборки для отслеживания ошибок
#
################################################################################
...
echo "   Вы увидите полный вывод сборки с прогресс-барами."
echo ""

# Полный вывод сборки без фильтрации - чтобы видеть прогресс и ошибки
docker build \
...
# Простое ожидание завершения create-site контейнера
# docker compose logs -f выйдет когда контейнер остановится
...
CREATE_SITE_EXIT=$(docker inspect frappe_docker_test-create-site-1 --format='{{.State.ExitCode}}' 2>/dev/null || echo "1")
...
docker exec frappe_docker_test-backend-1 bench --site localhost list-apps 2>/dev/null || echo "❌ Не удалось получить список"

echo ""
echo "🔍 Проверка DocTypes модуля Documents:"
docker exec frappe_docker_test-backend-1 bench --site localhost mariadb -e "SELECT name FROM tabDocType WHERE module='Documents';" 2>/dev/null || echo "❌ Не удалось проверить"

echo ""
echo "🔍 Проверка Folder Structure Templates:"
docker exec frappe_docker_test-backend-1 bench --site localhost mariadb -e "SELECT COUNT(*) as 'FST Count' FROM \`tabFolder Structure Template\`;" 2>/dev/null || echo "❌ Не удалось проверить"

echo ""
echo "🔍 Проверка Workspace:"
docker exec frappe_docker_test-backend-1 bench --site localhost mariadb -e "SELECT name FROM tabWorkspace WHERE name LIKE '%Document%';" 2>/dev/null || echo "❌ Не удалось проверить"
...
echo ""
echo "════════════════════════════════════════════════════════════════════════════"
echo "  📋 Скрипт install_1.sh создан!"
echo "════════════════════════════════════════════════════════════════════════════"
echo ""
echo "Для запуска установки выполните:"
echo "  ~/install_1.sh"
echo ""
